  ...
  useEffect(() => {
    async function loadDashboardData() {
      ...
      try {
        ...
        // 2. Fetch Manager Profile - CASTED TO ANY
        ...
        // 3. Fetch TODAY'S Delivery Metrics - CASTED TO ANY
  ...
  if (!metadata_id) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', textAlign: 'center', gap: 16 }}>
      ...
      <p style={{ color: 'var(--grey)', fontSize: 14, maxWidth: 400, lineHeight: 1.6 }}>
        ...
        Please contact <strong>Attract Acquisition</strong> support.
      </p>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 1000, margin: '0 auto', paddingBottom: 60 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          <h1 style={{ fontFamily: 'Playfair Display', fontSize: 32, fontWeight: 700, margin: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg2)', padding: '8px 16px', borderRadius: 20, border: '1px solid var(--border2)' }}>
          ...
          <span style={{ fontSize: 10, fontFamily: 'DM Mono', color: 'var(--teal)', fontWeight: 700 }}>
            LIVE ENGINE: {metrics?.date_key || 'WAITING'}
          </span>
        </div>
      </div>
      ...
      <div className="card" style={{ background: '#0a0f0e', border: '1px solid #1a2522', padding: '40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 32 }}>
          <span style={{ fontSize: 12, fontFamily: 'DM Mono', color: 'var(--grey)', letterSpacing: '0.2em' }}>
            DAILY PERFORMANCE — {client?.business_name?.toUpperCase() || 'CLIENT'}
          </span>
          <span style={{ fontSize: 10, color: 'var(--teal)', fontWeight: 700 }}>● {metrics?.date_key ? 'UPDATED' : 'WAITING FOR DATA'}</span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, marginTop: 60, borderTop: '1px solid #141c1a', paddingTop: 40 }}>
           <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 36, fontWeight: 700, color: 'var(--teal)', fontFamily: 'Playfair Display' }}>
                {metrics?.appointments_booked > 0 ? '81%' : '0%'}
              </div>
          ...
          {manager ? (
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
                <a href={`tel:${manager.phone || ''}`} className="btn-secondary" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 12, textDecoration: 'none', color: 'inherit', padding: '10px 0', border: '1px solid var(--border2)', borderRadius: 6 }}>
                  <Phone size={14} /> Call
                </a>
                <a href={`https://wa.me/${manager.phone || ''}`} target="_blank" rel="noreferrer" className="btn-secondary" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 12, textDecoration: 'none', color: 'inherit', padding: '10px 0', border: '1px solid var(--border2)', borderRadius: 6 }}>
              ...
              <div style={{ borderTop: '1px solid var(--border2)', paddingTop: 20 }}>
                ...
                <textarea 
                  ...
                  style={{ fontSize: 12, minHeight: 80, marginBottom: 10, width: '100%', background: 'var(--bg2)', border: '1px solid var(--border2)', borderRadius: 6, padding: 8, color: 'white' }}
                />
                <button 
                  onClick={submitReview} 
                  disabled={submitting}
                  className="btn-primary" 
                  style={{ width: '100%', fontSize: 12, opacity: submitting ? 0.5 : 1 }}
                >
