                    ...
                    <>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        ...
                        <div>
                          ...
                          <input className="input" type="date" defaultValue={selected.contract_start_date || ''} key={`start-${selected.id}`} onBlur={e => saveField(selected.id, 'contract_start_date', e.target.value)} />
                        </div>
                        <div>
                          <div className="label">Contract End</div>
                          <input className="input" type="date" defaultValue={selected.contract_end_date || ''} key={`end-${selected.id}`} onBlur={e => saveField(selected.id, 'contract_end_date', e.target.value)} />
                        </div>
                        <div>
                          <div className="label">Last Results Meeting</div>
                          <input className="input" type="date" defaultValue={selected.last_results_meeting || ''} key={`meeting-${selected.id}`} onBlur={e => saveField(selected.id, 'last_results_meeting', e.target.value)} />
                        </div>
                      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {[
        ...
        <div key={x.f}>
          ...
          <input className="input" value={(form as any)[x.f]}
      ...
      <div>
        ...
        <select className="input" value={form.tier} onChange={e => onTierChange(e.target.value)}>
          {TIERS.map(t => <option key={t.value} value={t.value}>{t.label} — R{t.setup.toLocaleString()} + R{t.retainer.toLocaleString()}/mo</option>)}
        </select>
      </div>
      <div>
      ...
      <div>
        ...
        <input className="input" placeholder="act_123456789" value={form.meta_ad_account_id}
          onChange={e => setForm(p => ({ ...p, meta_ad_account_id: e.target.value }))} />
      </div>
