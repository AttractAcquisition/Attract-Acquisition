      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'relative', background: 'var(--bg2)', padding: 28, width: 480, borderRadius: 8 }}>
            ...
            <AddProspectForm
              onSave={(p: Prospect) => { setProspects([p, ...prospects]); setShowAdd(false); toast('Prospect added') }}
...
function F({ label, field, value, onSave, type = 'text' }: {
  label: string; field: string; value: any; onSave: (v: string) => void; type?: string
}) {
  return (
    <div>
      <div className="label">{label}</div>
      <input className="input" type={type} defaultValue={value ?? ''} key={`${field}-${value}`}
        onBlur={e => { if (String(e.target.value) !== String(value ?? '')) onSave(e.target.value) }} />
    </div>
  )
}

function AddProspectForm({ onSave, onCancel }: { onSave: (p: Prospect) => void; onCancel: () => void }) {
  const { role, metadata_id } = useAuth()
  const [name, setName] = useState('')

  async function save() {
    if (!name) return
    const insertData: any = { business_name: name, status: 'new', city: 'Cape Town' }
    if (role === 'operator' && metadata_id) {
      insertData.assigned_to = metadata_id
    }
    const { data } = await supabase.from('prospects').insert(insertData).select().single()
    if (data) onSave(data)
  }
