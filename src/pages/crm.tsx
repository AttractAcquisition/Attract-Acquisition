...
export default function CRM() {
  ...
  async function load() {
    setLoading(true)
    // Force the query to return our custom Prospect type to resolve Property 'pipeline_stage' errors
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
        <div style={{ display: 'flex', gap: 8, background: 'var(--bg2)', padding: 4, borderRadius: 8 }}>
          <button onClick={() => setView('board')} className={view === 'board' ? 'btn-primary' : 'btn-ghost'} style={{fontSize: 11}}>Active Pipeline</button>
          <button onClick={() => setView('archive')} className={view === 'archive' ? 'btn-primary' : 'btn-ghost'} style={{fontSize: 11}}>Archive</button>
        </div>
            ...
            <div style={{ padding: 12, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {prospects.filter(p => p.pipeline_stage === stage).map(p => (
                <div key={p.id} className="card" onClick={() => setSelected(p)} 
                  style={{ padding: 14, background: 'var(--bg3)', cursor: 'pointer', border: '1px solid var(--border2)' }}>
