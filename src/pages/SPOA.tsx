...
export default function SPOA() {
  ...
  async function generateSPOA() {
    ...
    try {
      ...
      if (error) {
        // Surface the edge function's own error message if available
        const detail =
          typeof (error as Record<string, unknown>).context === 'object'
            ? JSON.stringify((error as Record<string, unknown>).context)
            : error.message
        throw new Error(detail)
      }
      if (!data?.success || !data.body_html) {
        throw new Error(data?.error ?? 'Edge function returned no content')
      }

      console.log('[SPOA] body_html received, length:', data.body_html.length)

      // ── Assemble full HTML document locally (styles live here, not in LLM) ──
      const year = new Date().getFullYear()
      const assembled = buildFullDocument(data.body_html, data.preview_stats.ref, year)

      console.log('[SPOA] Full document assembled, total length:', assembled.length)

      // Build blob URL for print window
      if (blobUrlRef.current) URL.revokeObjectURL(blobUrlRef.current)
  ...
  const printReport = useCallback(() => {
    if (!blobUrlRef.current) {
      toast('Generate a SPOA first', 'error')
      return
    }
    ...
    console.log('[SPOA] Print window opened')
  }, [])
                      ...
                      <div style={{
                        ...
                        <div style={{
                          ...
                          <span style={{
                            ...
                            LIVE PREVIEW — {spoaResult.preview_stats.ref}
                          </span>
                          <span style={{ fontFamily: 'DM Mono', fontSize: 9, color: 'var(--grey)' }}>
                            Use Print / PDF for best results
                          </span>
                        </div>
                        <ReportIframe
                          html={fullHtml}
                          title="SPOA Live Preview"
                          height={880}
                        />
                      </div>
                    )}
