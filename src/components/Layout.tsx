...
export default function Layout() {
  ...
  // Derive title — handle /sprints/:id
  ...
  const filteredNav = NAV
    .filter(group => {
      if (role === 'operator') return !['Finance', 'Settings'].includes(group.section)
      return true
    })
    .map(group => ({
      ...group,
      items: group.items.filter(item => {
        if (role === 'operator') return !['/scraper', '/capital'].includes(item.path)
        return true
      })
    }))
