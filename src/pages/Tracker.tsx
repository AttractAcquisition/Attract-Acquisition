...
export default function Tracker() {
  ...
  const initIdx    = Math.max(0, MONTHS.findIndex(m => m.key === todayMonthKey))
    ...
    <div style={{ maxWidth: 760 }}>
      ...
      {currentMonth.milestone && (
        <div style={{ background: 'var(--teal-faint)', border: '1px solid var(--teal-border)', borderRadius: 4, padding: '12px 18px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontFamily: 'DM Mono', fontSize: 11, letterSpacing: '0.1em', color: 'var(--teal)', textTransform: 'uppercase' }}>
            {currentMonth.milestone}
          </span>
        </div>
      )}
        ...
        : days.length === 0
          ? (
            <div className="empty-state">
              ...
              <p>Tasks will appear here once seeded from the master calendar.</p>
            </div>
          )
          : days.map(day => (
            <div key={day} style={{ marginBottom: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                ...
                {isToday(day) && (
                  <span style={{ fontFamily: 'DM Mono', fontSize: 9, letterSpacing: '0.08em', textTransform: 'uppercase', background: 'var(--teal)', color: 'var(--bg)', borderRadius: 3, padding: '2px 8px' }}>
                    Today
                  </span>
                )}
              ...
              {grouped[day].map(task => (
                <div key={task.id} onClick={() => toggleTask(task)}
                  style={{
                    ...
                    opacity: task.status === 'complete' ? 0.45 : 1,
                  }}
                ...
                >
                  ...
                  <div style={{
                    flex: 1, fontSize: 14, color: 'var(--white)', lineHeight: 1.5,
